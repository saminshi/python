...
@LastEditTime: 2019-11-07 21:48:40
@Description: example
...
def command_dict_gereate(frame = 0,slot = 1,pid = 0,ontid = 0,*args,**kwargs):
    ...
    command_dicts[0]= [
        'interface gpon 0/{}'.format(bid),
        'display ont ver {} {}'.format(portid,ontid)
        ]

    command_dicts[1]= [
        'interface gpon111 0/{}'.format(bid),
        'display ont ver1111 {} {}'.format(portid,ontid)
        ]
    
    command_dicts[2]= [
        'interface gpon2222 0/{}'.format(bid),
        'display ont ver2222 {} {}'.format(portid,ontid)
        ]
    command_dicts[3]= [
        'interface gpon2222 0/{}'.format(bid),
        'lineprofile profile {}'.format(line_profile_id),
        'srvprofile profile {}'.format(srv_profile_id)
        ]
